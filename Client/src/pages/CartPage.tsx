...
const CartPage: React.FC = () => {
    ...
    const fetchCartProducts = async () => {
      try {
        ...
        const response = await axios.get(
          "https://codsoft-e-commerce-website-nine.vercel.app/auth/cart",
          {
            headers: {
  ...
  const handleDeleteProduct = async (productId: number) => {
    try {
      ...
      await axios.delete(
        `https://codsoft-e-commerce-website-nine.vercel.app/auth/cart/${productId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleProceedToPayment = async () => {
    try {
      ...
      if (usermail === "" || totalCost === null) {
        ...
        toast.error(
          "Unable to proceed to payment. Missing user information or total cost."
        );
        return;
      }
          ...
          <div className="mt-8 proceedtopayment-logic">
            {totalPrice !== null && (
              <div className="space-y-3">
                ...
                <p className="text-xl font-semibold">
                  Tax (8%): ${(totalPrice * TAX_RATE).toFixed(2)}
                </p>
                <p className="text-xl font-semibold">
